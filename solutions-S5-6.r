...
set.seed(4)
M <- list()
for (i in 1:10) M[[i]] <- matrix(runif((i + 1)^2) - .5, i + 1, i + 1)
## a
names(M) <- paste("M", 2:11, sep = "")
## b
nM <- sapply(M, "norm", type = "F")
## c
svdM <- lapply(M, svd)
## d
macp <- function(A) {
  ## find product of col max abs for A
  prod(apply(A, 2, function(a) max(abs(a))))
}
## check...
macp(M[[1]])
prod(max(abs(M[[1]][, 1]))) * prod(max(abs(M[[1]][, 2])))
## e
sapply(M, macp)
